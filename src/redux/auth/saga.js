...
function* loginWithEmailPassword({ payload }) {
  ...
  try {
    ...
    if (!loginUser.message) {
      const item = { uid: loginUser.user.uid, ...getCurrentUser()};
      const response = yield call(getUserForLogin, item);
      setCurrentUser(response);
      console.log("response", response)
      history.push(adminRoot);
      yield put(loginUserSuccess(response));
    } else {
      const errorMessage = loginUser.code
        ? FIREBASE_ERROR[loginUser.code]
        : FIREBASE_ERROR.default;
      yield put(loginUserError(errorMessage));
    }
  } catch (error) {
    const errorMessage = error.code
      ? FIREBASE_ERROR[error.code]
      : FIREBASE_ERROR.default;
    yield put(loginUserError(errorMessage));
  }
}
